...
if (!supabaseUrl || !supabaseKey) {
  console.warn("Missing SUPABASE_URL or SUPABASE_SERVICE_ROLE_KEY env vars — running in introspection-only mode");
}
...
export async function getFollowUpSearches(
  researchIds: string[],
  excludeQuery?: string,
  limit: number = 5
  ...
  const { data, error } = await supabase.rpc("get_follow_up_searches", {
    p_research_ids: researchIds,
    p_exclude_query: excludeQuery ?? null,
    p_limit: limit,
  });

  if (error) {
    console.error("Follow-up searches error:", error.message);
    return [];
  }

  return (data ?? []) as FollowUpSearch[];
}
